...
## インストール済みスキルとの優先ルール
以下のルールはCLAUDE.mdのルールを優先する（スキルの指示より上位）：

- **`using-superpowers`**：「全レスポンス前に実行」は適用しない。CLAUDE.mdの報告フォーマットに従い日本語で直接回答する。ユーザーが明示的に呼び出した場合のみ実行。
- **`verification-before-completion`**：Unityプロジェクトではエディタでの確認が必要なため、CLIからの検証コマンドで完了を判断しない。「Unityで確認してほしいこと」を明示してユーザーに渡す。
- **`writing-plans`**：新機能実装前はCLAUDE.mdの「新機能実装前ルール4ステップ」を使用する（既存例調査→難易度比較→簡易版提案→ゲームループ基準）。スキルのプラン形式は使用しない。
- **`systematic-debugging`**：バグ調査後の報告はCLAUDE.mdの報告フォーマット（結論→動作説明→技術補足→修正内容→確認事項）に従う。
- **`simplify`**：ユーザーが明示的に要求した場合のみ実行。タスク範囲外のリファクタリングは行わない。

## Superpowersスキルの使い分け（現在無効化中）

■ 重い・複雑なシステム（Inventory・Magazine・状態遷移・AI）
→ systematic-debugging / verification-before-completion を積極利用

■ 軽い機能（演出・UI微修正・猫・足跡など）
→ using-superpowersを抑えて軽量運用

## Superpowers導入前に知っておくべきリスク

### トークン使用量の急増
- systematic-debuggingは関連ファイルを横断探索するため
  1回の修正で通常の3〜5倍のトークンを消費する可能性がある
- verification-before-completionは確認項目を列挙するため
  レスポンスが長くなりトークンを消費する
- using-superpowersは毎回スキルチェックを走らせるため
  軽い修正でも余分なトークンが発生する

### 対策
- 軽い修正は冒頭に「systematic-debuggingは使わずに対応して」と明示
- 複雑なシステム修正のみ積極利用
- 使用量が週の80%を超えたら軽量運用に切り替える

## 調査・実装の原則
- 必要ファイルだけ読む
- 不要な全体探索を避ける
- 影響範囲を最初に宣言する
- 人間側がどこを調査・検証させるかを設計する
...
- BoxGrid武器ammo追跡実装済み（`Inventory.MoveToBoxItem/MoveToBox`にWeaponブランチ追加。`AddWeaponToBox(name,ammo)`でammo付き登録。PlayerGrid/装備スロット→BoxGrid移動でammo保持。`DropTarget.cs`のEquipmentSlot→BoxGridパスも`GetSavedAmmo()`使用に修正。`MoveAllToPlayer`もWeapon対応。`AddWeaponToBox`にLootContainer/StorageContainer同期追加）
...
## ItemData カテゴリ
```csharp
public enum ItemCategory { Weapon, Consumable, Bullet, Magazine }
// Magazine専用フィールド: maxAmmo / compatibleWeapon / compatibleBullet
```
